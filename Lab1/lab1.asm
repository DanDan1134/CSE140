...
li a7, 4                      # print string syscall to a7 (4 is for printing string)
ecall
...
li a1, 5                      # loads immediate 5 (length) into a1 to use during bubble sort 
call bubble_sort
##################################################
# Print sorted
##################################################
...
li t2, 0                      # index = 0
jal print_loop
li a7, 10                     # exit program syscall (finishes code after printing "print done")
ecall
############################################################
# bubble_sort
############################################################
...
blez t0, done                 # skip if empty/one (<= 0)
outer_loop:
...
ble t3, t4, no_swap           # skip if in order (t3 <= t4)
...
beqz t6, done                 # done if no swaps (t6 is 0 or 1 if a swap occured)
sub t0, t0, a6                # outer limit--
bgtz t0, outer_loop           # repeat outer loop (if t0 > 0)
done:
ret
############################################################
# print messages
...
li a7, 1                      # print integer syscall (1 is for printing integer)
ecall
la a0, space                  # load space string
li a7, 4                      # print string
ecall
addi t0, t0, 4                # next array element (increments 4 bytes to next int)
addi t2, t2, 1                # i++ to the t2 counter
j print_loop
print_done:
la a0, newline                # load newline
li a7, 4                      # print string
ecall
ret                           # returns to jump address